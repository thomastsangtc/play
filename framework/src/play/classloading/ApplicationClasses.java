...
public class ApplicationClasses {
    ...
    public static class ApplicationClass {
         ...
         */
        public byte[] enhance() {
            if (Arrays.equals(enhancedByteCode, javaByteCode)) {
            ...
            if (isClass()) {
                ...
                try {
                    CtClass ctClass = enhanceChecker_classPool.makeClass(new ByteArrayInputStream(this.javaByteCode));
	...
	public static boolean isClass(String name) {
         ...
         */
        public byte[] compile() {
            // Do not even try to compile source file if it's not a real class (its potential source file is already compiled)
            if (name.contains("$") && (name.endsWith("_") || name.endsWith("BeanInfo") || name.endsWith("Customizer"))) {
                // inexisting class (generated by hibernate?)
                return null;
            }

            String baseClassName = name.contains("$") ? name.substring(0, name.indexOf('$')) : name;
            if (Play.classes.getApplicationClass(baseClassName).compiled)
              return javaByteCode;
